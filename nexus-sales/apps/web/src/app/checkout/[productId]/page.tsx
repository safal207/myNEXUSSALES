...
export default function CheckoutPage() {
  ...
  const abVariant = searchParams.get('v') || 'A';
  ...
  const onSubmit = async (data: CheckoutFormValues) => {
    ...
    try {
      ...
      track('order_created', { orderId: result.order.id, productId, abVariant });
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="w-full max-w-lg p-8 space-y-6 bg-white rounded-xl shadow-lg animate-pulse">
          ...
          <div className="h-12 bg-gray-300 rounded w-1/3 mx-auto mt-4"></div>
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen text-center p-10">
        ...
        <button onClick={fetchProduct} className="mt-6 py-2 px-5 bg-indigo-600 text-white rounded-md font-semibold hover:bg-indigo-700">
          Retry
        </button>
      </div>
    );
      ...
      <div className="w-full max-w-lg p-8 space-y-6 bg-white rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input id="email" type="email" {...register('email')} onChange={() => track('start_checkout', { productId, abVariant })} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md" />
          ...
          <button type="submit" disabled={isSubmitting} data-ab-variant={abVariant} className="w-full py-3 px-4 bg-indigo-600 text-white rounded-md font-semibold hover:bg-indigo-700 disabled:bg-indigo-400">
